...
class InvitationCodesController < ApplicationController
  ...
  def index
    @invitation_codes = InvitationCode.where(applicant_id: current_user.id).order('id desc').paginate page: params[:page]
    @today_code = CodeLog.ensure_only(current_user)
    respond_with @invitation_codes
  end
